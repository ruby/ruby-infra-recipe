...
`s3.tf` manages the three public S3 buckets the services here use as origins (`ftp.r-l.o`, `rubyci`, `blade.ruby-lang.org`) through the AWS provider, so `terraform plan` needs AWS credentials for account 317825688411 beyond what the state backend already requires. Bucket ACLs and public access block settings are intentionally left out.
